// server.js — BRICS Unit Basket v4 (simulation + multi-leg, v2-compatible JSON)
...
function computeUnitPrice(){
  ...
  const fx_usd_twap = {};
  FX_LIST.forEach(c=>{
    fx_usd_twap[c] = goldUSD * FX_WEIGHT / GOLD_WEIGHT; // same proportion as before
  });

  const unitUSD = goldLegUSD + FX_LIST.reduce((sum,c)=>sum + fx_usd_twap[c],0);
...
app.listen(PORT, ()=>console.log(`BRICS Unit Basket v4 running on port ${PORT}`));
